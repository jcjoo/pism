...
export function Sales() {
  ...
  const handleFetchSales = async () => {
    ...
    try {
      ...
      if (filterProduct)                results = results.filter(s => s.sale_items.some((i: any) => i.product_id === filterProduct.id));
  ...
  const handleDelete = async () => {
    const ok = await confirm({ title: 'Apagar Venda', message: 'Deseja apagar esta venda?', confirmText: 'Apagar', destructive: true });
    if (!ok) return;
    setLoading(true);
    try {
      await salesService.delete(selectedSale.id);
      toast.show('Venda apagada!', { type: 'success' });
      setStep('filter'); setSalesData([]);
  ...
  const openEditReceipt = () => {
    ...
    setReceiptAmount((selectedSale.received_amount ?? calcTotal(selectedSale.sale_items)).toFixed(2).replace('.', ','));
  ...
  const handleRemoveReceipt = async () => {
    const ok = await confirm({ title: 'Remover recebimento', message: 'Deseja desfazer este recebimento?', confirmText: 'Remover', destructive: true });
    if (!ok) return;
    setSavingReceipt(true);
    try {
  ...
  const handleMarkReceived = () => {
    ...
    setReceiptAmount(calcTotal(selectedSale.sale_items).toFixed(2).replace('.', ','));
    setReceiptModalVisible(true);
  };
  ...
  const totalReceived = salesData.filter(s => s.received_at).reduce((a, s) => a + (s.received_amount ?? calcTotal(s.sale_items)), 0);
  const totalPending  = salesData.filter(s => !s.received_at).reduce((a, s) => a + calcTotal(s.sale_items), 0);

  const openDate = (target: string) => { setDateFieldTarget(target); setShowDatePicker(true); };
  ...
  return (
    <View className="screen">
      <FormScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 100 }}>
        {step === 'filter' && (
          <SaleFilterStep
            filterClient={filterClient} filterProduct={filterProduct}
            statusFilter={statusFilter} dateStart={dateStart} dateEnd={dateEnd}
            dueStart={dueStart} dueEnd={dueEnd} loading={loading}
            onOpenModal={(type) => { setModalType(type); setModalVisible(true); }}
            onOpenDate={openDate}
        ...
        {step === 'list' && (
          <SaleListStep
            salesData={salesData} totalAll={totalAll}
            totalReceived={totalReceived} totalPending={totalPending}
            onSelectSale={(sale) => { setSelectedSale(sale); setStep('details'); }}
            onBack={() => setStep('filter')}
            onNewSearch={() => { setStep('filter'); setSalesData([]); }}
          />
        )}

        {step === 'details' && selectedSale && (
          <SaleDetailsStep
            sale={selectedSale} savingReceipt={savingReceipt}
            onBack={() => setStep('list')}
            onEdit={startEdit}
        ...
        {step === 'edit' && selectedSale && (
          <SaleEditStep
            sale={selectedSale} editCart={editCart} setEditCart={setEditCart}
            editDueDate={editDueDate} editPaymentMode={editPaymentMode}
            editObservation={editObservation} setEditObservation={setEditObservation}
            loading={loading} onSave={handleSaveEdit}
            onCancel={() => setStep('details')} onDelete={handleDelete}
            switchPayment={switchPayment} onOpenDatePicker={openDate}
          />
        )}
      </FormScrollView>
