...
export function SelectorModal({ visible, type, clients, products, onSelect, onClose }: SelectorModalProps) {
  ...
  const data     = isClient
    ? [{ id: 'none', name: 'Todos os Clientes' } as any, ...clients]
    : [{ id: 'none', name: 'Todas as Mercadorias' } as any, ...products];
      ...
      <View className="modal-overlay-center">
        <View className="modal-panel-center">
          <Text className="modal-title">
            Selecione {isClient ? 'o Cliente' : 'a Mercadoria'}
          </Text>
          <FlatList
            data={data}
            keyExtractor={i => i.id}
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                onPress={() => onSelect(item.id === 'none' ? null : item)}
              >
                <Text className="text-base text-primary">{item.name}</Text>
              </TouchableOpacity>
            )}
            ListEmptyComponent={<Text className="empty-text">Nenhum item.</Text>}
          />
